...
void PWM_init() {
  ...
  //PORT->Group[g_APinDescription[8].ulPort].PMUX[g_APinDescription[8].ulPin >> 1].reg |= /*PORT_PMUX_PMUXO_F |*/ PORT_PMUX_PMUXO_F;
...
void updatedac() {
  if (awgon) {
    ...
    if (awgpwnon) {
      ...
      //analogWrite(8, awgoutb[m]); // pwm(A10, pwmf, awgoutb[m]);
      m++;
    }
  } else {
...
void setup() {
  ...
  const int inputPins[numberOfInputPins] = {2, 3, 4, 5, 6, 7}; // pins to use as inputs
    ...
    if (SerialUSB.available()){
      ...
      switch (c){
        ...
        case 't': // change the Scope value of st in uSec
          st2 = SerialUSB.parseInt();
          if(st2>0){
            st=st2;
          }
          break;
        case 'b': // change number of samples to capture
          bs = SerialUSB.parseInt();
          if(bs>bmax){
            bs=bmax;
          }
          break;
        ...
        case 'm': // change pwm duty cycle % 
          pwid = SerialUSB.parseInt(); // range from 0 (0%) to 1000 (100%)
          TCC0->CTRLBSET.reg = TCC_CTRLBSET_LUPD; // Set the Lock Update bit
          while (TCC0->SYNCBUSY.bit.CTRLB);       // Wait for synchronization
          TCC0->PERB.reg = pwmf;                  // Set the frequency of the PWM on TCC0
          while(TCC0->SYNCBUSY.bit.PERB);         // Wait for synchronization
